...
path1="../immediate_vis/yjc/pixel6.png"
path2="../immediate_vis/yjc/gt_new/mask_visualization3.png"
path4="../immediate_vis/yjc/gt/mask_visualization3.png"
path3="../immediate_vis/yjc/pred/mask_visualization3.png"
...
specific_image_path = "/home/Tarkiya/project/NLP/code/yjc/data/ADEChallengeData2016/images/validation/ADE_val_00000005.jpg"
...
with torch.no_grad():
    ...
    # 应用visual_prediction_forward_convnext处理
...
# 8. 提取对应类别的文本嵌入
# 获取num_templates信息，这是每个类别对应的模板数量
meta = batched_input.get("meta", {"dataname": "openvocab_ade20k_panoptic_val"})
dataname = meta.get('dataname', "openvocab_ade20k_panoptic_val")
...
for i, num in enumerate(num_templates):
    if i > 0:
        class_start_indices.append(class_start_indices[-1] + num)

# 提取每个类别的文本嵌入
text_embeddings = []
for label in unique_labels:
    label_idx = label.item()
    if label_idx < len(class_start_indices):
        start_idx = class_start_indices[label_idx]
        if label_idx + 1 < len(class_start_indices):
            end_idx = class_start_indices[label_idx + 1]
        else:
            end_idx = start_idx + num_templates[label_idx]
        
        # 获取该类别的所有模板嵌入并平均
        class_embeddings = text_classifier[start_idx:end_idx]
        text_embeddings.append(class_embeddings.mean(dim=0))
    else:
        print(f"Warning: Label {label_idx} exceeds available classes")
        # 使用零向量作为占位符
        text_embeddings.append(torch.zeros_like(text_classifier[0]))

text_embeddings = torch.stack(text_embeddings)

num_classes = len(unique_labels) # 定义num_classes
...
text_features_scaled = scaler_text.fit_transform(text_embeddings_np)
# text_features_scaled = text_embeddings.cpu().numpy()


# 10.2 对像素特征进行标准化
clip_features_sampled_np = clip_features_sampled.cpu().numpy()
pixel_features_scaled = scaler_pixels.fit_transform(clip_features_sampled_np)
# pixel_features_scaled = clip_features_sampled.cpu().numpy()
...
# 12. 分离文本嵌入和像素特征的2D坐标
text_coords = features_2d[:num_classes]  # 前num_classes个是文本嵌入
pixel_coords = features_2d[num_classes:]  # 后面的是像素特征
...
# 过滤掉背景类的像素点
valid_pixel_mask = torch.ones_like(gt_labels_sampled, dtype=torch.bool)
for label in [255]:  # 可以添加其他需要过滤的标签
    valid_pixel_mask &= (gt_labels_sampled != label)

# 绘制文本嵌入（三角形）
for i, (label_idx, color) in enumerate(zip(unique_labels, colors)):
    class_name = class_name_dict.get(label_idx.item(), f"Class {label_idx.item()}")
    plt.scatter(text_coords[i, 0], text_coords[i, 1], 
...
plt.title('TSNE Visualization: Text Embeddings vs Pixel Features')
